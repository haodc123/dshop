...
class ModelPurchasePurchase extends Model {
    ...
    public function addPurchase($data) {
        if ($data['receipt_delivery'] == "receipt") {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "purchase` SET reference_no = '" . $this->db->escape($data['reference_no']) . "', warehouse_id = '" . (int) $data['warehouse_id'] . "', supplier_id = '" . (int) $data['supplier_id'] . "', supplier = '" . $this->db->escape($data['supplier']) . "', note= '" . $this->db->escape($data['note']) . "', total = '" . (float) $data['total'] . "', quantity = '" . (int) $data['total_quantity'] . "', purchase_status_id =1, language_id='" . (int) $this->config->get('config_language_id') . "', date_added= '" . $this->db->escape(date('Y-m-d', strtotime(str_replace('/', '-', $data['date_added']) ? str_replace('/', '-', $data['date_added']) : $data['date_added']))) . "', date_modified = NOW()");
        ...
        }elseif ($data['receipt_delivery'] == "delivery") {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "purchase` SET reference_no = '" . $this->db->escape($data['reference_no']) . "', warehouse_id = '" . (int) $data['warehouse_id'] . "', supplier_id = '" . (int) $data['supplier_id'] . "', supplier = '" . $this->db->escape($data['supplier']) . "', note= '" . $this->db->escape($data['note']) . "', total = '" . (float) $data['total'] . "', quantity = '" . (int) $data['total_quantity'] . "', purchase_status_id =1, language_id='" . (int) $this->config->get('config_language_id') . "', date_added= '" . $this->db->escape(date('Y-m-d', strtotime(str_replace('/', '-', $data['date_added']) ? str_replace('/', '-', $data['date_added']) : $data['date_added']))) . "', date_modified = NOW()");
            $purchase_id = $this->db->getLastId();
            if (isset($data['purchase_product'])) {
                foreach ($data['purchase_product'] as $purchase_product) {
    ...
    public function editPurchase($purchase_id, $data) {
        ...
        if ($purchase_query->num_rows) {
            if ($purchase_query->row['receipt_delivery'] == 'receipt') {
            ...
            if ($purchase_query->row['receipt_delivery'] == 'delivery') {
                $product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "purchase_product WHERE purchase_id = '" . (int) $purchase_id . "'");
    ...
    public function deletePurchase($purchase_id) {
        ...
        if ($purchase_query->num_rows && $purchase_query->row['receipt_delivery']=='receipt') {
          ...
          if ($purchase_query->num_rows && $purchase_query->row['receipt_delivery']=='delivery') {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "purchase_product WHERE purchase_id = '" . (int) $purchase_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "purchase_option WHERE purchase_id = '" . (int) $purchase_id . "'");
    }
    ...
    public function getPurchase($purchase_id) {
        ...
        if ($purchase_query->num_rows) {
            return array(
                ...
                'receipt_delivery' => $purchase_query->row['receipt_delivery']
            );
        } else {
            return false;
        }
    ...
    public function getPurchases($data = array()) {
        ...
        $sql = "SELECT DISTINCT * FROM " . DB_PREFIX . "purchase WHERE language_id='" . (int) $this->config->get('config_language_id') . "' and status=1";
        ...
        if (!empty($data['filter_receipt_delivery'])) {
            $sql .= " AND receipt_delivery = '" . $this->db->escape($data['filter_receipt_delivery']) . "'";
        }
...
//		}
    ...
    public function getTotalPurchases($data = array()) {
        $sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "purchase`  WHERE language_id='" . (int) $this->config->get('config_language_id') . "' AND status=1";
    ...
    public function getPurchaseOptions($purchase_id, $purchase_product_id) {
