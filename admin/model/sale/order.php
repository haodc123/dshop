...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_date_added'])) {
			$sql .= " AND DATE(o.date_added) = DATE('" . $this->db->escape(date('Y-m-d',strtotime(str_replace('/','-', $data['filter_date_added'])? str_replace('/','-', $data['filter_date_added']): $data['filter_date_added']))) . "')";
		}   
		
		if (!empty($data['filter_date_modified'])) {
			$sql .= " AND DATE(o.date_modified) = DATE('" . $this->db->escape(date('Y-m-d',strtotime(str_replace('/','-', $data['filter_date_modified'])? str_replace('/','-', $data['filter_date_modified']): $data['filter_date_modified']))) . "')";
		}
