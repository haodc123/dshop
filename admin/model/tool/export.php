...
class ModelToolExport extends Model {
	...
	function storeProductsIntoDatabase( &$database, &$products ) 
	{
		...
		foreach ($products as $product) {
			...
			$quantity = $product['quantity'];
                        $mcs = $database->escape($product['mcs']);
                        $color = $database->escape($product['color']);
			...
			$sql .= "`stock_status_id`,`mcs`,`color`,`size`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
			...
			$sql .= "$stockStatusId,'$mcs','$color','$size','$model',$manufacturerId,'$imageName',$shipping,$price,$points,";
	...
	function populateProductsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('quantity'),4)+1);
                $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('mcs'),8)+1);
                $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('color'),8)+1,$textFormat);
                $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('size'),8)+1,$textFormat);
		...
		$this->setCell( $worksheet, $i, $j++, 'quantity', $boxFormat );
                $this->setCell( $worksheet, $i, $j++, 'mcs', $boxFormat );
                $this->setCell( $worksheet, $i, $j++, 'color', $boxFormat );
                $this->setCell( $worksheet, $i, $j++, 'size', $boxFormat );
		...
		foreach ($products as $row) {
			...
			$this->setCell( $worksheet, $i, $j++, $row['mcs'] );
                        $this->setCell( $worksheet, $i, $j++, $row['color']);
                        $this->setCell( $worksheet, $i, $j++, $row['size']);
		...
		try {
			...
			header('Content-Disposition: attachment;filename="list_products.xlsx"');
		...
		try {
			...
			// redirect output to client browser
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="list_categories.xlsx"');
			header('Cache-Control: max-age=0');
			$objWriter = PHPExcel_IOFactory::createWriter($workbook, 'Excel2007');
			$objWriter->save('php://output');
