<?php
class ControllerPurchaseDelivery extends Controller {

    private $error = array();

    public function index() {
        $this->language->load('purchase/delivery');
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('purchase/delivery');
        $this->getList();
    }
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_purchase_delivery->addDelivery($this->request->post);
            ...
            $this->redirect($this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    public function update() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_purchase_delivery->editDelivery($this->request->get['purchase_id'], $this->request->post);
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && ($this->validateDelete())) {
            foreach ($this->request->post['selected'] as $purchase_id) {
                $this->model_purchase_delivery->deleteDelivery($purchase_id);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            ...
            $this->model_purchase_delivery->deleteDelivery($this->request->get['purchase_id']);
            $this->session->data['success'] = $this->language->get('text_success');
    ...
    protected function getList() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );

        $this->data['invoice'] = $this->url->link('purchase/delivery/invoice', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['insert'] = $this->url->link('purchase/delivery/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['delete'] = $this->url->link('purchase/delivery/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['confirm'] = $this->url->link('purchase/delivery/confirm', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $purchase_total = $this->model_purchase_delivery->getTotalDeliveries($data);
        $results = $this->model_purchase_delivery->getDeliveries($data);
        $i = 1;
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('purchase/delivery/invoice', 'token=' . $this->session->data['token'] . '&purchase_id=' . $result['purchase_id'] . $url, 'SSL')
            );
            $action[] = array(
                'index' => 1,
                'text' => $this->language->get('text_edit'),
                'href' => $this->url->link('purchase/delivery/update', 'token=' . $this->session->data['token'] . '&purchase_id=' . $result['purchase_id'] . $url, 'SSL')
            );
            $action[] = array(
                'index' => 2,
                'text' => $this->language->get('text_delete'),
                'href' => $this->url->link('purchase/delivery/delete', 'token=' . $this->session->data['token'] . '&purchase_id=' . $result['purchase_id'] . $url, 'SSL')
            );
              $action[] = array(
                'index' => 3,
                'text' => $this->language->get('text_comfirm'),
                'href' => $this->url->link('purchase/delivery/confirm', 'token=' . $this->session->data['token'] . '&purchase_id=' . $result['purchase_id'] . $url, 'SSL')
            );
            $this->data['purchases'][] = array(
                'index' => $i++,
        ...
        $this->data['sort_quantity'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=quantity' . $url, 'SSL');
        $this->data['sort_purchase_id'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=purchase_id' . $url, 'SSL');
        $this->data['sort_reference_no'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=reference_no' . $url, 'SSL');
        $this->data['sort_total'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=total' . $url, 'SSL');
        $this->data['sort_date_added'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
        $this->data['sort_date_modified'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . '&sort=date_modified' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->template = 'purchase/delivery_list.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function getForm() {
        ...
        if (!isset($this->request->get['purchase_id'])) {
            $this->data['action'] = $this->url->link('purchase/delivery/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('purchase/delivery/update', 'token=' . $this->session->data['token'] . '&purchase_id=' . $this->request->get['purchase_id'] . $url, 'SSL');
        }

        $this->data['cancel'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . $url, 'SSL');

        if (isset($this->request->get['purchase_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $purchase_info = $this->model_purchase_delivery->getDelivery($this->request->get['purchase_id']);
        }
        ...
        if (isset($this->request->post['reference_no'])) {
            ...
            $this->data['reference_no'] = $this->config->get('config_delivery_prefix') . $this->model_purchase_delivery->getTotalDate($this->data['date_added']);
        }
        ...
        $purchase_products = $this->model_purchase_delivery->getDeliveryProducts($this->data['purchase_id']);
        ...
        $this->template = 'purchase/delivery_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function addProduct() {
        ...
        $this->receipt = new Delivery($this->registry);
        if ($this->user->isLogged() && $this->user->hasPermission('modify', 'purchase/delivery')) {
            // Product
            $this->load->model('purchase/delivery');
            if (isset($this->request->post['product_id'])) {
                $product_info = $this->model_purchase_delivery->getProduct($this->request->post['product_id']);
                if ($product_info) {
                    if (isset($this->request->post['quantity'])) {
            ...
            if (isset($this->request->post['purchase_product'])) {
                foreach ($this->request->post['purchase_product'] as $purchase_product) {
                    $product_info = $this->model_purchase_delivery->getProduct($purchase_product['product_id']);
                    if ($product_info) {
    ...
    public function barcode() {
        ...
        if ($this->user->isLogged() && $this->user->hasPermission('modify', 'purchase/delivery')) {
            ...
            if (isset($this->request->get['barcode'])) {
                $product_info = $this->model_purchase_delivery->getProductBarcode($this->request->get['barcode']);
    ...
    protected function validateForm() {
        if (!$this->user->hasPermission('modify', 'purchase/delivery')) {
    ...
    public function info() {
        ...
        $purchase_info = $this->model_purchase_delivery->getDelivery($purchase_id);
        ...
        if ($purchase_info) {
            ...
            $this->data['invoice'] = $this->url->link('purchase/delivery/invoice', 'token=' . $this->session->data['token'] . '&purchase_id=' . (int) $this->request->get['purchase_id'], 'SSL');
            $this->data['cancel'] = $this->url->link('purchase/delivery', 'token=' . $this->session->data['token'] . $url, 'SSL');
            $this->data['purchase_id'] = $this->request->get['purchase_id'];
            ...
            $purchases = $this->model_purchase_delivery->getDeliveryProducts($this->request->get['purchase_id']);
            ...
            $this->template = 'purchase/delivery_info.tpl';
            $this->children = array(
                'common/header',
                'common/footer'
            );
    ...
    public function createInvoiceNo() {
        ...
        if (!$this->user->hasPermission('modify', 'purchase/delivery')) {
            ...
            $invoice_no = $this->model_purchase_delivery->createInvoiceNo($this->request->get['purchase_id']);
    ...
    public function addCredit() {
        ...
        if (!$this->user->hasPermission('modify', 'purchase/delivery')) {
            ...
            if ($purchase_info && $purchase_info['supplier_id']) {
                ...
                $credit_total = $this->model_sale_supplier->getTotalTransactionsByDeliveryId($this->request->get['purchase_id']);
    ...
    public function addReward() {
        ...
        if (!$this->user->hasPermission('modify', 'purchase/delivery')) {
            ...
            if ($purchase_info && $purchase_info['supplier_id']) {
                ...
                $reward_total = $this->model_sale_supplier->getTotalCustomerRewardsByDeliveryId($this->request->get['purchase_id']);
    ...
    public function addCommission() {
        ...
        if (!$this->user->hasPermission('modify', 'purchase/delivery')) {
            ...
            if ($purchase_info && $purchase_info['affiliate_id']) {
                ...
                $affiliate_total = $this->model_sale_affiliate->getTotalTransactionsByDeliveryId($this->request->get['purchase_id']);
    ...
    public function invoice() {
        ...
        foreach ($purchases as $purchase_id) {
            ...
            $purchase_data = array();
            $purchases = $this->model_purchase_delivery->getDeliveryProducts($purchase_id);
            foreach ($purchases as $purchase) {
                $purchase_data[] = array(
                    'name' => $purchase['name'],
                    'model' => $purchase['model'],
        ...
        $this->template = 'purchase/delivery_invoice.tpl';
        $this->response->setOutput($this->render());
    }
...
public function confirm() {
        ...
        if (isset($this->request->post['selected']) && ($this->validateConfirm())) {
            foreach ($this->request->post['selected'] as $purchase_id) {
                $this->model_purchase_delivery->confirmDelivery($purchase_id);
            }
            ...
            $this->model_purchase_delivery->comfirmDelivery($this->request->get['purchase_id']);
            $this->session->data['success'] = $this->language->get('text_success');
