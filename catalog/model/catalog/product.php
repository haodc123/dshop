...
class ModelCatalogProduct extends Model {
    ...
    public function getProducts($data = array()) {
        ...
        $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND p.status = '1' AND p.display = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
        ...
        if (!empty($data['filter_min']) || !empty($data['filter_max'])) {
            if ($data['filter_min'] == 0 && $data['filter_max'] != 0) {
                $sql .= " AND p.price <'" . $data['filter_max'] . "'";
            } else if ($data['filter_max'] == 0 && $data['filter_min'] != 0) {
                $sql .= " AND p.price >'" . $data['filter_min'] . "'";
            } else if ($data['filter_min'] != 0 && $data['filter_max'] != 0) {
                $sql .= " AND p.price >= '" . $data['filter_min'] . "' AND p.price <= '" . $data['filter_max'] . "'";
            }
        }
        if (!empty($data['filter_color'])) {
            $sql .= " AND p.color='" . $data['filter_color'] . "'";
        }
        ...
        if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            if (!empty($data['filter_tag'])) {
             ...
             if (!empty($data['filter_name'])) {
                $sql .= " OR LCASE(p.mc) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
            }
            if (!empty($data['filter_name'])) {
                $sql .= " OR LCASE(p.mcs) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
            }
...
//              
//             $product_to_category = $this->db->query("SELECT p.*, pd.* FROM " . DB_PREFIX . "product p LEFT JOIN  " . DB_PREFIX . "product_description pd on p.product_id=pd.product_id LEFT JOIN  " . DB_PREFIX . "product_to_category ptc ON p.product_id=ptc.product_id WHERE ptc.category_id IN(select category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id='" . (int) $product_id . "') group by p.product_id");
//            $product_data = array();
//             foreach ($product_to_category->rows as $result) { 		
//			$product_data[$result['product_id']] = $this->getProduct($result['product_id']);
//		}
//                
//                return $product_data ;
//         }
    ...
    public function getProductModel($model) {
        $sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.model = '" . $this->db->escape($model) . "' AND p.status='1' AND  p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
        $sql .= " GROUP BY p.mc";
        $query = $this->db->query($sql);
        return $query->rows;
    }

    public function getProductSize($mc) {
        $sql = "SELECT p.product_id as product_id, s.code as code , s.name as name  FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "size s ON (p.size = s.code) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.mc = '" . $this->db->escape($mc) . "' AND p.status=1 AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
        $query = $this->db->query($sql);
        return $query->rows;
    }
    ...
    public function getNextProduct($product_id) {
        ...
        $query = $this->db->query("SELECT next.product_id as product_id FROM " . DB_PREFIX . "product current JOIN " . DB_PREFIX . "product next ON next.product_id > current.product_id WHERE current.product_id ='" . (int) $product_id . "' ORDER BY next.product_id ASC LIMIT 1 ");
        return $query->row;
    }

    public function getPrevProduct($product_id) {

        $query = $this->db->query("SELECT prev.product_id as product_id FROM " . DB_PREFIX . "product current JOIN " . DB_PREFIX . "product prev ON prev.product_id < current.product_id WHERE current.product_id ='" . (int) $product_id . "' ORDER BY prev.product_id DESC LIMIT 1");
