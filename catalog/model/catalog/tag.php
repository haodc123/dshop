...
class ModelCatalogTag extends Model {
    ...
    public function getTags($data) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tag WHERE language_id = '" . (int) $this->config->get('config_language_id') . "' ORDER BY RAND() LIMIT " . (int) $data['limit']);
        return $query->rows;
    }

    public function getTag($tag_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "tag WHERE tag_id = '" . (int) $tag_id . "' AND language_id = '" . (int) $this->config->get('config_language_id') . "'");
        return $query->row;
    }
    ...
    public function getProducts($data = array()) {
        ...
        $sql .= " FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_tag pt ON p.product_id=pt.product_id";
        $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
        if (!empty($data['filter_tag_id'])) {
            $sql .= " AND pt.tag_id = '" . (int) $data['filter_tag_id'] . "'";
        }
    ...
    public function getTotalProducts($data = array()) {
        ...
        $sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_tag pt ON p.product_id=pt.product_id";
        $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
