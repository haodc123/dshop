...
class ControllerAccountWishList extends Controller {
	public function index() {
		...
		foreach ($this->session->data['wishlist'] as $key => $product_id) {
			...
			if ($product_info) { 
                            $color= $this->model_catalog_color->getColorCode($product_info['color']);
                            $size= $this->model_catalog_color->getSizeCode($product_info['size']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/wishlist.tpl')) {
			...
			$this->template = 'default/template/account/wishlist.tpl';
                 ...
                 if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/not_found_wishlist.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/error/not_found_wishlist.tpl';
		} else {
			$this->template = 'default/template/error/not_found_wishlist.tpl';
		}
                }
	...
	public function add(){
        ...
        if (isset($this->request->post['remove'])) {
          ...
          unset($this->session->data['wishlist'][$key]);
            ...
            $json['empty'] =$this->language->get('text_empty');
        }
        $json['total'] = sprintf($this->language->get('text_wishlist'), (isset($this->session->data['wishlist']) ? count($this->session->data['wishlist']) : 0));
    }
        $this->response->setOutput(json_encode($json));
