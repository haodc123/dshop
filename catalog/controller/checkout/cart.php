...
//            unset($this->session->data['reward']);
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $this->data['continue'] = $this->url->link('common/home');
            unset($this->session->data['success']);
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/not_found_cart.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/error/not_found_cart.tpl';
            } else {
                $this->template = 'default/template/error/not_found_cart.tpl';
            }
            $this->children = array(
                'common/column_left',
                'common/column_right',
            ...
            if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
                ...
                foreach ($products as $product) {
                    ...
                    if ($product['image']) {
                        ...
                        $image = $this->model_tool_image->resize('no-image.jpg', $this->config->get('config_image_cart_width'), $this->config->get('config_image_cart_height'));
                    }
                ...
                $json['key'] = $this->request->post['remove'];
                $json['totals'] = $total_data;
            } else {

                $json['empty'] = true;
            }
            $json['total_quantity'] = sprintf($this->language->get('text_items'), $this->cart->countProducts());
        }
        ...
        if (!empty($this->request->post['quantity']) && !empty($this->request->post['key'])) {
            ...
            foreach ($products as $product) {
                ...
                if ($product['key'] == $this->request->post['key']) {
                    ...
                    // Display total prices
                    if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                        $json['total'] = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
                    } else {
                        $json['total'] = false;
                    }
                    $json['stock'] = $product['stock'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning'));
    ...
    public function smallCart() {
        ...
        $json['checkout'] = $this->url->link('checkout/cart', '', 'SSL');
        $this->response->setOutput(json_encode($json));
    }
    ...
    public function add() {
        ...
        if ($product_info) {
            ...
            if (!$json) {
                if ($this->cart->hasIdStock($this->request->post['product_id'])) {
                    ...
                    $json['error_warning'] = $product['stock'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning'));
                    $json['totals'] = $total_data;
                    //  $json['total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total));
                    $json['checkout'] = $this->url->link('checkout/cart');
                    $json['total_quantity'] = sprintf($this->language->get('text_items'), $this->cart->countProducts());
                }
            } else {
    ...
    public function validate() {
       ...
       if (!$this->cart->hasStock() && (!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning'))) {
        ...
        if (!$this->customer->isLogged()) {
            ...
            $json['login'] = $this->url->link('checkout/checkout');
        }
        $this->response->setOutput(json_encode($json));
    }
